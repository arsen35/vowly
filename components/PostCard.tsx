...
export const PostCard: React.FC<PostCardProps> = ({ post, onLike, onAddComment, onDelete, isAdmin }) => {
  ...
  // DINAMIK AVATAR DUZELTICI: Eski (pembe) verileri marka renklerine zorla
  const brandAvatar = post.user.avatar.includes('ui-avatars.com') 
    ? post.user.avatar.replace(/background=[^&]*/, 'background=A66D60').replace(/color=[^&]*/, 'color=fff')
    : post.user.avatar;
    ...
    <div className="bg-white dark:bg-theme-dark sm:rounded-xl shadow-sm border-y sm:border border-gray-100 dark:border-gray-800 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full transform hover:scale-[0.98]">
      ...
      <div className="p-3 flex items-center gap-3 border-b border-gray-50 dark:border-gray-800 relative">
        <img 
          src={brandAvatar} 
          alt={post.user.name} 
          className="w-9 h-9 rounded-full object-cover border border-wedding-200 dark:border-gray-700 shadow-sm"
        />
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-wedding-900 dark:text-white text-sm truncate tracking-tight">{post.user.name}</h3>
          <p className="text-[10px] font-semibold text-wedding-500/80 dark:text-gray-500 uppercase tracking-widest">
        ...
        {isAdmin && (
            <div className="relative" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-100 dark:border-gray-700 z-20 py-1 animate-in fade-in zoom-in-95 duration-200">
                        <button onClick={handleDelete} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">Sil</button>
                    </div>
                )}
            </div>
        )}
      ...
      <div className="p-3 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            <button onClick={handleLike} className={`${isLiked ? 'text-wedding-500' : 'text-gray-900 dark:text-white'}`}>
            ...
            <button onClick={() => setIsCommentOpen(!isCommentOpen)} className="text-gray-900 dark:text-white">
        ...
        <div className="text-gray-900 dark:text-gray-100 text-sm mb-2 line-clamp-2 hover:line-clamp-none cursor-pointer">
          <span className="font-bold mr-1">{post.user.name}</span> {post.caption}
        </div>
        
        <div className="flex flex-wrap gap-1 mb-2 mt-auto">
          {post.hashtags.slice(0, 3).map((tag, i) => (
            <span key={i} className="text-wedding-500 text-[10px] font-medium bg-wedding-50 dark:bg-wedding-900/20 px-1.5 py-0.5 rounded">#{tag.replace('#', '')}</span>
          ))}
        </div>

        {isCommentOpen && (
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 mt-2 border border-gray-100 dark:border-gray-800 animate-fadeIn">
            ...
            <div className="flex gap-1.5 items-center">
              <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmitComment()} placeholder="Yorum..." className="flex-1 bg-white dark:bg-gray-700 border border-gray-200 rounded-full px-3 py-1 text-xs outline-none" />
              <button onClick={handleSubmitComment} disabled={!commentText.trim()} className="text-wedding-500 font-bold text-xs">Paylaş</button>
            </div>
          </div>
        )}
      </div>
