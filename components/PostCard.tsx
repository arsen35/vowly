...
export const PostCard: React.FC<PostCardProps> = ({ 
  ...
  return (
    <div className="bg-white dark:bg-theme-black sm:rounded-md border-y sm:border border-gray-100 dark:border-zinc-900 overflow-hidden flex flex-col h-full transition-all duration-200">
      {/* HEADER */}
      <div className="px-3 py-2 flex items-center justify-between border-b border-gray-50 dark:border-zinc-900/50">
        <div className="flex items-center gap-2">
          <img 
            src={post.user.avatar} 
            alt={post.user.name} 
            className="w-8 h-8 rounded-sm object-cover border border-black/5 dark:border-white/5"
          />
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5 leading-none">
              <h3 className="font-bold text-gray-900 dark:text-white text-[12px] tracking-tight">{post.user.name}</h3>
              {post.location && (
                  <span className="text-[10px] text-gray-400 font-medium tracking-tight">• {post.location}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {!isOwnPost && (
            <button 
              onClick={onFollow}
              className={`text-[11px] font-bold transition-colors ${isFollowing ? 'text-gray-400' : 'text-wedding-500 hover:text-wedding-600'}`}
            >
          ...
          {isAdmin && (
              <div className="relative" ref={menuRef}>
                  ...
                  {isMenuOpen && (
                      <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-zinc-900 rounded-md shadow-2xl border border-gray-100 dark:border-zinc-800 z-20 py-1 animate-in fade-in zoom-in-95">
                          <button onClick={() => { setIsMenuOpen(false); onDelete(post.id); }} className="w-full text-left px-4 py-2 text-[10px] text-red-600 font-bold uppercase tracking-wider">Sil</button>
                      </div>
                  )}
              </div>
          )}
      ...
      >
        {showBigHeart && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30 animate-like-bounce">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-20 h-20 text-white drop-shadow-2xl opacity-90">
      ...
      {/* SHOP BUTTON */}
      {post.productUrl && (
          <a href={post.productUrl} target="_blank" className="w-full px-4 py-3 bg-gray-50 dark:bg-zinc-900/50 flex items-center justify-between text-[10px] font-bold text-gray-900 dark:text-gray-200 uppercase tracking-widest border-b border-gray-100 dark:border-zinc-900/50 transition-colors hover:bg-gray-100 dark:hover:bg-zinc-900">
             <div className="flex items-center gap-2">
      ...
      <div className="p-3 pt-2 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3.5">
            <div className="flex items-center gap-1">
                ...
                >
                  <svg fill={isLiked ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    <path d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                  </svg>
                </button>
                <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-tight">{likesCount}</span>
            </div>
            ...
            <div className="flex items-center gap-1">
                ...
                <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-tight">{post.comments.length}</span>
            </div>
        ...
        {isCommentOpen && (
          <div className="bg-gray-50 dark:bg-zinc-900/30 rounded-md p-2.5 mt-3 border border-gray-100 dark:border-zinc-800 animate-fadeIn">
            <div className="space-y-1.5 mb-2.5 max-h-24 overflow-y-auto custom-scrollbar px-1">
               {post.comments.map((comment) => (
                 <div key={comment.id} className="text-[10px] pb-1 border-b border-gray-100 dark:border-zinc-800/50 last:border-0">
            ...
            <div className="flex gap-2 items-center bg-white dark:bg-theme-black rounded px-1 py-1 ring-1 ring-gray-100 dark:ring-zinc-800">
              <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmitComment()} placeholder="Yorum yap..." className="flex-1 bg-transparent px-2 text-[10px] outline-none dark:text-white" />
              <button onClick={handleSubmitComment} disabled={!commentText.trim()} className="text-wedding-500 font-bold text-[9px] px-2 disabled:opacity-30 uppercase tracking-widest text-center">Paylaş</button>
            </div>
          </div>
        )}
      </div>
