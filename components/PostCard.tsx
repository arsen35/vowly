            ...
            <div className="relative" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-100 dark:border-gray-700 z-20 py-1 animate-in fade-in zoom-in-95 duration-200">
                        <button onClick={handleDelete} className="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-red-50 flex items-center gap-2 font-medium">Gönderiyi Sil</button>
                    </div>
                )}
            </div>
        )}
      ...
      >
        {showBigHeart && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30 animate-like-bounce">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-20 h-20 text-white drop-shadow-xl filter">
        ...
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleShare} className="text-gray-900 dark:text-white transition-transform active:scale-90">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 -mt-1 -rotate-45">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
               </svg>
            </button>
          </div>
          <div className="font-bold text-xs text-gray-900 dark:text-white tracking-tight">{likesCount} beğenme</div>
        </div>
        
        <div className="text-gray-900 dark:text-gray-100 text-[13px] mb-2 leading-relaxed line-clamp-2 hover:line-clamp-none cursor-pointer">
          <span className="font-bold mr-1">{post.user.name}</span> {post.caption}
        </div>
        
        <div className="flex flex-wrap gap-1.5 mb-2.5 mt-auto">
          {post.hashtags.slice(0, 3).map((tag, i) => (
            <span key={i} className="text-wedding-500 text-[9px] font-bold bg-wedding-50 dark:bg-wedding-900/20 px-1.5 py-0.5 rounded-md">#{tag.replace('#', '')}</span>
          ))}
        </div>

        {isCommentOpen && (
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-2.5 mt-2 border border-gray-100 dark:border-gray-800 animate-fadeIn text-xs">
            <div className="space-y-1 mb-2 max-h-32 overflow-y-auto custom-scrollbar">
               {post.comments.map((comment) => (
                 <div key={comment.id} className="pb-1 border-b border-gray-100 dark:border-gray-700/50 last:border-0">
            ...
            <div className="flex gap-2 items-center">
              <input 
                ...
                className="flex-1 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full px-3 py-1 text-[11px] outline-none focus:border-wedding-500" 
              />
              <button onClick={handleSubmitComment} disabled={!commentText.trim()} className="text-wedding-500 font-bold text-[11px] disabled:opacity-50">Paylaş</button>
            </div>
          </div>
        )}
      </div>
