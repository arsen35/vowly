...
export const ChatPage: React.FC<ChatPageProps> = ({ isAdmin }) => {
  ...
  if (!isNameSet) {
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 animate-fadeIn">
            <div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm w-full text-center border border-wedding-100">
                <div className="w-16 h-16 bg-wedding-100 rounded-full flex items-center justify-center mx-auto mb-4 text-wedding-600">
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-[#efe7dd] relative">
        {/* Background Pattern */}
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#881337 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>

        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar z-10">
            {messages.length === 0 && (
                <div className="text-center py-10 opacity-50">
                    <p className="text-gray-600 font-serif">Henüz mesaj yok. İlk mesajı sen yaz! 👋</p>
                </div>
            )}
                    ...
                    <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex max-w-[80%] md:max-w-[60%] gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`w-8 h-8 shrink-0 ${isPrevSame ? 'opacity-0 h-0' : ''}`}>
                                <img src={msg.avatar} className="w-8 h-8 rounded-full border border-black/10 shadow-sm bg-white" alt="avatar" />
                            </div>
                            ...
                            <div className={`
                                ...
                                ${isMe 
                                    ? 'bg-[#E1FFC7] text-gray-800 rounded-l-xl rounded-br-xl' 
                                    : msg.isAdmin 
                                        ? 'bg-wedding-50 border border-wedding-200 text-gray-900 rounded-r-xl rounded-bl-xl'
                                        : 'bg-white text-gray-800 rounded-r-xl rounded-bl-xl'
                                }
                                ${isPrevSame ? (isMe ? 'rounded-tr-xl mt-0.5' : 'rounded-tl-xl mt-0.5') : 'rounded-t-xl mt-2'}
                            `}>
                                {!isMe && !isPrevSame && (
                                    <p className={`text-[10px] font-bold mb-0.5 ${msg.isAdmin ? 'text-wedding-600 flex items-center gap-1' : 'text-orange-600'}`}>
                                        {msg.userName}
                                        {msg.isAdmin && (
                                            <span className="bg-wedding-600 text-white text-[8px] px-1 rounded-sm uppercase tracking-wider">Yönetici</span>
                                        )}
                                    </p>
                                )}
        ...
        {/* Input Area */}
        <div className="bg-white p-3 md:p-4 border-t border-gray-200 z-20 shadow-lg">
            <form onSubmit={handleSendMessage} className="flex gap-2 max-w-4xl mx-auto">
                <input 
                    type="text" 
                    className="flex-1 bg-gray-100 border-0 rounded-full px-5 py-3 text-sm focus:ring-2 focus:ring-wedding-300 focus:bg-white transition-all outline-none"
                ...
                <button 
                    ...
                    className="bg-wedding-500 hover:bg-wedding-700 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-md disabled:opacity-50 disabled:shadow-none transition-all transform active:scale-95"
                >
