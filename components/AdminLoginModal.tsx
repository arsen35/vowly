...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({ onClose, onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        // Not: Eğer bu mail adresi Firebase'de yoksa veya farklı bir şifreye sahipse 
        // signInWithEmailAndPassword hata verecektir. Yönetici hesaplarının 
        ...
        if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
            ...
            setError('Yönetici girişi şu an yapılamıyor. Bir hata oluştu.');
        }
    } finally {
        setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 bg-black/80 z-[2000] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in">
      <div className="bg-[#121212] border border-red-900/30 rounded-2xl w-full max-w-sm shadow-2xl p-8 relative animate-in zoom-in-95">
        ...
        <div className="text-center mb-8">
            ...
            <h2 className="text-xl font-serif font-bold text-white tracking-tight uppercase">Yönetici Yetkilendirmesi</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full py-4 rounded-xl shadow-none bg-red-600 hover:bg-red-700" isLoading={isLoading}>Yönetici Olarak Gir</Button>
        </form>
      </div>
    </div>
  );
