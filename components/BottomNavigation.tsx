...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ currentView, onNavigate, onUploadClick }) => {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[50] px-5 pb-8 pt-2 pointer-events-none">
      <div className="bg-white/40 dark:bg-black/40 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-[12px] flex items-center justify-around h-16 pointer-events-auto max-w-sm mx-auto shadow-[0_8px_32px_rgba(0,0,0,0.1)]">
        
        {/* AKIŞ (HOME) */}
        <button 
          onClick={() => onNavigate(ViewState.FEED)}
          className={`flex flex-col items-center justify-center flex-1 h-full transition-all ${currentView === ViewState.FEED ? 'text-wedding-500' : 'text-gray-500 dark:text-gray-400'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full transition-all ${currentView === ViewState.BLOG ? 'text-wedding-500' : 'text-gray-500 dark:text-gray-400'}`}
        >
        ...
        <div className="flex-1 flex justify-center items-center">
            <button 
              ...
              className="w-11 h-11 bg-wedding-500 text-white rounded-[10px] flex items-center justify-center transform active:scale-90 transition-all shadow-lg shadow-wedding-500/20"
            >
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full transition-all ${currentView === ViewState.CHAT ? 'text-wedding-500' : 'text-gray-500 dark:text-gray-400'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center flex-1 h-full transition-all ${currentView === ViewState.PROFILE ? 'text-wedding-500' : 'text-gray-500 dark:text-gray-400'}`}
        >
