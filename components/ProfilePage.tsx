  ...
  if (!user) {
      return (
          <div className="max-w-md mx-auto px-6 py-20 text-center animate-fadeIn">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-10 leading-relaxed italic text-center text-pretty">
                  Annabella topluluğuna katılmak ve anılarını paylaşmak için giriş yap.
              </p>
    ...
    <div className="max-w-4xl mx-auto px-4 pt-6 md:pt-12 pb-24 animate-fadeIn">
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex items-center gap-4 md:gap-14">
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-0">
                ...
                <p className="hidden md:block text-[11px] dark:text-gray-400 leading-snug font-light italic mt-1">
      ...
      {selectedPost && (
          <div className="fixed inset-0 z-[150] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="w-full max-w-sm md:max-w-md max-h-[90vh] overflow-y-auto custom-scrollbar bg-white dark:bg-theme-black rounded-md shadow-2xl animate-in slide-in-from-bottom-5">
                  <PostCard 
                    post={selectedPost} 
                    onLike={onLike} 
      ...
      {isFollowModalOpen && (
          <div className="fixed inset-0 z-[1500] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
              <div className="bg-white dark:bg-[#121212] w-full max-w-sm rounded-lg overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 max-h-[70vh]">
                  <div className="p-4 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center bg-gray-50/50 dark:bg-zinc-900/50">
                      <h3 className="font-serif font-bold dark:text-white text-xs uppercase tracking-widest">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                      {isFollowListLoading ? (
                          ...
                          followListUsers.map(listUser => (
                              <div key={listUser.id} className="flex items-center justify-between animate-fadeIn bg-gray-50/50 dark:bg-zinc-900/50 p-2 rounded-md border border-gray-100 dark:border-zinc-800/50">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex flex-col">
                                          ...
                                          <span className="text-[9px] text-gray-400 truncate max-w-[120px] italic">Gelin adayı ✨</span>
                                      </div>
                                  </div>
                                  <button 
      ...
      {showSettings && (
          <div className="fixed inset-0 bg-black/60 z-[1100] flex items-end animate-in fade-in" onClick={() => setShowSettings(false)}>
              <div className="w-full bg-white dark:bg-[#121212] rounded-t-lg p-6 pb-12 space-y-2.5" onClick={e => e.stopPropagation()}>
                  <button onClick={onInstallApp} className="w-full text-center py-4 text-xs font-bold text-wedding-500 border border-wedding-100 dark:border-wedding-900/20 rounded-md bg-wedding-50/50 dark:bg-wedding-900/10 uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" /></svg>
                      Uygulamayı Yükle
                  </button>
                  <button onClick={onLogout} className="w-full text-center py-4 text-xs font-bold text-gray-900 dark:text-white border border-gray-100 dark:border-zinc-800 rounded-md bg-gray-50 dark:bg-zinc-900/50 uppercase tracking-[0.2em]">Oturumu Kapat</button>
                  <button onClick={() => { setShowSettings(false); onDeleteAccount(); }} className="w-full text-center py-4 text-xs font-bold text-red-500 border border-red-100 dark:border-red-900/10 bg-red-50 dark:bg-red-900/10 rounded-md uppercase tracking-[0.2em]">Hesabı Sil</button>
        ...
        <div className="fixed inset-0 bg-black/80 z-[2000] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in">
            <div className="bg-white dark:bg-theme-dark rounded-lg w-full max-w-sm p-8 animate-in zoom-in-95 relative shadow-2xl">
                ...
                <div className="space-y-6">
                    <div className="flex flex-col items-center gap-3">
                        <div 
                            ...
                            className="relative w-24 h-24 rounded-md border-2 border-wedding-500/30 p-1 group cursor-pointer overflow-hidden"
                        >
                            <img src={editAvatar} className="w-full h-full rounded-md object-cover" alt="edit-avatar" />
                ...
                <div className="mt-8 flex flex-col gap-2">
                    ...
                    <button onClick={() => setIsEditModalOpen(false)} className="w-full py-2 text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] text-center">İptal</button>
                </div>
            </div>
        </div>
      )}
