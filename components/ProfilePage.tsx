...
export const ProfilePage: React.FC<ProfilePageProps> = ({ 
  ...
  if (!user) {
      return (
          <div className="max-w-md mx-auto px-6 py-20 text-center animate-fadeIn">
              <div className="w-24 h-24 bg-gray-50 dark:bg-zinc-900 rounded-lg flex items-center justify-center mx-auto mb-8 border border-gray-100 dark:border-zinc-800 shadow-inner">
              ...
              <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-white mb-3 text-center">Profiline Eriş</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-10 leading-relaxed italic text-center">
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-6 md:pt-12 pb-24 animate-fadeIn">
      ...
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex items-center gap-4 md:gap-14">
            <div className="shrink-0">
                <div className="w-20 h-20 md:w-32 md:h-32 rounded-lg p-[2px] border border-wedding-500/20 overflow-hidden shadow-sm">
            ...
            <div className="flex flex-col gap-2 flex-1 min-w-0">
                ...
                <div className="flex gap-4 md:gap-6 overflow-x-auto no-scrollbar">
                    <div className="flex flex-col md:flex-row md:items-center">
                        ...
                        <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold md:ml-1 mt-1 md:mt-0">Gönderi</span>
                    </div>
                    <div className="flex flex-col md:flex-row md:items-center cursor-pointer hover:opacity-70 transition-opacity" onClick={() => setIsFollowModalOpen('followers')}>
                        ...
                        <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold md:ml-1 mt-1 md:mt-0">Takipçi</span>
                    </div>
                    <div className="flex flex-col md:flex-row md:items-center cursor-pointer hover:opacity-70 transition-opacity" onClick={() => setIsFollowModalOpen('following')}>
                        ...
                        <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold md:ml-1 mt-1 md:mt-0">Takip</span>
                    </div>
                </div>
                
                <p className="hidden md:block text-[13px] dark:text-gray-400 leading-snug font-light italic">
        ...
        <div className="flex gap-2">
            <button 
                ...
                className="flex-1 bg-transparent border border-gray-200 dark:border-zinc-800 hover:border-wedding-500 hover:text-wedding-500 text-gray-500 dark:text-gray-400 text-[10px] font-bold py-3 rounded-lg transition-all uppercase tracking-[0.1em]"
            >
                Düzenle
            </button>
            ...
            <button 
                ...
                className={`flex-1 bg-transparent border ${activeTab === 'liked' ? 'border-wedding-500 text-wedding-500' : 'border-gray-200 dark:border-zinc-800 text-gray-500 dark:text-gray-400'} hover:border-wedding-500 hover:text-wedding-500 text-[10px] font-bold py-3 rounded-lg transition-all uppercase tracking-[0.1em]`}
            >
            ...
            <a 
                ...
                target="_blank"
                className="flex-1 bg-transparent border border-gray-200 dark:border-zinc-800 hover:border-wedding-500 hover:text-wedding-500 text-gray-500 dark:text-gray-400 text-[10px] font-bold py-3 rounded-lg transition-all uppercase tracking-[0.1em] text-center"
            >
                Mağaza
            </a>
        </div>
          ...
          <div key={post.id} className="aspect-square relative group cursor-pointer overflow-hidden bg-gray-50 dark:bg-zinc-900 shadow-inner">
            ...
            {activeTab === 'posts' && (
                <button 
                    ...
                    className="absolute top-2 right-2 p-1 bg-black/50 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 z-10"
                >
            ...
            {activeTab === 'liked' && (
                <button 
                    ...
                    className="absolute top-2 right-2 p-1 bg-black/50 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-wedding-500 z-10"
      ...
      {/* FOLLOWERS / FOLLOWING LIST MODAL - Düzeltilmiş Mantık */}
      {isFollowModalOpen && (
          <div className="fixed inset-0 z-[1500] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
              <div className="bg-white dark:bg-[#121212] w-full max-w-sm rounded-2xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 max-h-[70vh]">
                  <div className="p-4 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center">
                      <h3 className="font-serif font-bold dark:text-white text-md uppercase tracking-widest">
                              ...
                              <div key={listUser.id} className="flex items-center justify-between animate-fadeIn">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex flex-col">
                                          <span className="text-[13px] font-bold dark:text-white">{listUser.name}</span>
                                          <span className="text-[10px] text-gray-400 truncate max-w-[120px] italic">{listUser.bio || "Gelin adayı ✨"}</span>
                                      </div>
                                  </div>
                                  <button 
                                    onClick={() => onFollowToggle(listUser.id)}
                                    className={`text-[10px] font-bold px-4 py-1.5 rounded-md border transition-all uppercase tracking-widest ${followingIds.includes(listUser.id) ? 'bg-gray-50 dark:bg-zinc-900 border-gray-100 dark:border-zinc-800 text-gray-400' : 'bg-wedding-500 border-wedding-500 text-white'}`}
                                  >
                                      {followingIds.includes(listUser.id) ? 'Bırak' : 'Takip Et'}
                                  </button>
                              </div>
                          ))
                      ) : (
                          <div className="py-20 text-center opacity-30 italic font-serif text-sm">Henüz bir veri yok.</div>
                      )}
                  </div>
              </div>
          </div>
      ...
      {showSettings && (
          <div className="fixed inset-0 bg-black/60 z-[1100] flex items-end animate-in fade-in" onClick={() => setShowSettings(false)}>
              <div className="w-full bg-white dark:bg-[#121212] rounded-t-2xl p-6 pb-12 space-y-3" onClick={e => e.stopPropagation()}>
                  <button onClick={onLogout} className="w-full text-center py-4 text-xs font-bold text-gray-900 dark:text-white border border-gray-100 dark:border-zinc-800 rounded-xl bg-gray-50 dark:bg-zinc-900/50 uppercase tracking-[0.2em]">Oturumu Kapat</button>
                  <button onClick={() => { setShowSettings(false); onDeleteAccount(); }} className="w-full text-center py-4 text-xs font-bold text-red-500 border border-red-100 dark:border-red-900/10 bg-red-50 dark:bg-red-900/10 rounded-xl uppercase tracking-[0.2em]">Hesabı Sil</button>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-[2000] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in">
            <div className="bg-white dark:bg-theme-dark rounded-2xl w-full max-w-sm p-8 animate-in zoom-in-95 relative shadow-2xl">
                ...
                <div className="space-y-6">
                    <div className="flex flex-col items-center gap-3">
                        ...
                        >
                            <img src={editAvatar} className="w-full h-full rounded-md object-cover transition-transform group-hover:scale-110" alt="edit-avatar" />
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-lg px-4 py-3 text-sm dark:text-white outline-none focus:border-wedding-500 transition-colors" placeholder="Ad Soyad" />
                        </div>
                        <div>
                            <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1.5 ml-1">Biyografi</label>
                            <textarea value={editBio} onChange={(e) => setEditBio(e.target.value)} className="w-full bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-lg px-4 py-3 text-sm dark:text-white outline-none h-24 resize-none focus:border-wedding-500 transition-colors font-light italic" placeholder="Hayallerini anlat..." />
                        </div>
                    </div>
                </div>

                <div className="mt-8 flex flex-col gap-3">
                    <Button onClick={handleSaveProfile} isLoading={isSaving} className="w-full py-4 rounded-lg shadow-none">Değişiklikleri Kaydet</Button>
                    <button onClick={() => setIsEditModalOpen(false)} className="w-full py-2 text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] hover:text-red-500 transition-colors text-center">Vazgeç</button>
                </div>
            </div>
        </div>
      )}
