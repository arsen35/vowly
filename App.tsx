...
const App: React.FC = () => {
  ...
  // Global PWA Install Listener (Must be at top level)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
        ...
        if (user) {
            ...
            unsubFollow = dbService.subscribeToFollowData(user.uid, (data) => setFollowingIds(data.following));
  ...
  const handleNotificationClick = (notif: AppNotification) => {
      ...
      if (notif.type === 'dm') setViewState(ViewState.CHAT);
  ...
  const testNotification = () => {
    ...
    if (Notification.permission === "granted") new Notification(testNotif.senderName, { body: testNotif.message, icon: testNotif.senderAvatar });
  ...
  const handleLogout = async () => {
    if (!auth) return;
    try { await signOut(auth); setCurrentUser(null); setIsAdmin(false); setViewState(ViewState.FEED); } catch (e) { console.error(e); }
  };

  const handleDeleteAccount = async () => {
    if (!auth || !auth.currentUser) return;
    try { await deleteUser(auth.currentUser); setCurrentUser(null); setIsAdmin(false); setViewState(ViewState.FEED); alert("Hesabınız silindi."); } catch (e) { alert("Tekrar giriş yapıp deneyin."); }
  };

  const handleFollowToggle = async (targetUserId: string) => {
    if (!currentUser) { setViewState(ViewState.PROFILE); return; }
    const isFollowing = followingIds.includes(targetUserId);
    try { if (isFollowing) await dbService.unfollowUser(currentUser.id, targetUserId); else await dbService.followUser(currentUser.id, targetUserId); } catch (e) { console.error(e); }
  };
  ...
  const handleSendMessageClick = (user: User) => { setChatTargetUser(user); setViewState(ViewState.CHAT); };
  ...
  const handleNewPost = async (data: any) => {
    ...
    } catch (e) { alert("Paylaşım Hatası"); }
  };

  const handleLike = async (postId: string, isLikedNow: boolean) => {
    if (!currentUser) { setViewState(ViewState.PROFILE); return; }
    try { await dbService.toggleLike(postId, currentUser.id, isLikedNow); } catch (error) {}
  };

  const handleAddComment = async (postId: string, text: string) => {
    if (!currentUser) { setViewState(ViewState.PROFILE); return; }
    try { await dbService.addComment(postId, { id: Date.now().toString(), userId: currentUser.id, userName: currentUser.name, text, timestamp: Date.now() }); } catch(error) {}
  };
  ...
  return (
    <div className={`min-h-screen bg-white dark:bg-theme-black pb-24 md:pb-0 transition-colors duration-300 relative`}>
      <div className="fixed top-0 left-0 right-0 z-[4000] pointer-events-none flex flex-col items-center">
          {activeNotifications.map(n => <NotificationToast key={n.id} notification={n} onClose={removeNotification} onClick={handleNotificationClick} />)}
      </div>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-white/60 dark:bg-black/40 backdrop-blur-xl border-b border-white/20 dark:border-white/10 h-14">
        <div className="w-full h-full flex items-center justify-between px-4 md:px-[20px] lg:px-[60px] 2xl:px-[100px]">
          <div className="flex items-center cursor-pointer select-none" onClick={handleLogoClick}><Logo className="h-7 w-auto" /></div>
          <nav className="hidden md:flex items-center gap-10 absolute left-1/2 transform -translate-x-1/2">
               ...
               <button onClick={() => setViewState(ViewState.CHAT)} className={`relative text-[10px] font-bold tracking-[0.2em] transition-all uppercase ${viewState === ViewState.CHAT ? 'text-gray-900 dark:text-white' : 'text-gray-300 dark:text-zinc-600 hover:text-gray-900 dark:hover:text-white'}`}>SOHBET{unreadDMCount > 0 && <span className="absolute -top-1 -right-3 w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>}</button>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {isAdmin && <button onClick={() => setShowAdminModal(true)} className="flex items-center gap-2 text-[9px] font-bold text-red-500 uppercase tracking-widest border border-red-500/20 px-3 py-1.5 rounded-md hover:bg-red-500 hover:text-white transition-all shadow-sm">PANEL</button>}
            <button onClick={() => setShowSearchModal(true)} className="p-2 text-gray-400 hover:text-wedding-500 transition-all active:scale-90"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button>
      ...
      <main className="w-full pt-14">
        <div className={`px-0 md:px-[20px] lg:px-[60px] 2xl:px-[100px] ${viewState === ViewState.FEED ? 'pt-0' : 'pt-4'}`}>
          ...
          : viewState === ViewState.PROFILE ? <ProfilePage user={currentUser} currentUser={currentUser} isAdmin={isAdmin} onOpenAdmin={() => setShowAdminModal(true)} posts={posts} onPostClick={(p) => setViewState(ViewState.FEED)} onLogout={handleLogout} onDeleteAccount={handleDeleteAccount} onDeletePost={setPostToDelete} onLoginSuccess={() => setViewState(ViewState.FEED)} onLike={handleLike} onAddComment={handleAddComment} followingIds={followingIds} onFollowToggle={handleFollowToggle} onInstallApp={handleInstallApp} onUserClick={handleUserClick} onMessageClick={handleSendMessageClick} onTestNotification={testNotification} />
      ...
      <div className="hidden md:flex fixed bottom-8 right-8 flex-col gap-3 z-50">
        ...
        <button onClick={handleUploadClick} className="bg-gray-900 dark:bg-white p-3 rounded-lg shadow-xl text-white dark:text-gray-900 hover:scale-105 transition-all active:scale-95"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg></button>
      </div>
